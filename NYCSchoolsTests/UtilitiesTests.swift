...
class MyNYCSessionSuccessProtocol: NYCSessionProtocol {
    ...
    func dataTask(with request: URLRequest, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) -> NYCURLSessionDataTask {
        ...
        if let path = Bundle(for: WebServiceTests.self).path(forResource: resourceName, ofType: "json") {
            let data = try? Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe)
            completionHandler(data, UtilitiesTests.successHttpURLResponse(request: request), nil)
        }
...
class MyNYCSessionFailureProtocol: NYCSessionProtocol {
    ...
    func dataTask(with request: URLRequest, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) -> NYCURLSessionDataTask {
        completionHandler(nil, UtilitiesTests.wrongHttpURLResponse(request: request, statusCode: statusCode), error)
